...
public class Snapper_Script : MonoBehaviour
{
    ...
    void PlayerDetect()
    {
        ...
        if (Physics.Raycast(eyeball.transform.position, eyeball.transform.forward, out hit, sightRange, playerLayer))
        {
            ...
            {
                transform.position = Vector3.MoveTowards(this.transform.position, player.position, 7 * Time.deltaTime); //enemy start chasing
